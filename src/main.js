...
// استيراد المكتبات باستخدام ES Modules
...
// دالة لجلب بيانات الجرائد من صفحة معينة باستخدام Cheerio
...
export default async (req, res) => {
    try {
        ...
        if (latestDoc.documents && latestDoc.documents.length > 0) {
            // بما أن الأعداد مخزنة بصيغة مملوءة بالأصفار (مثل "001")، يمكن تحويلها لعدد
        ...
        for (const journal of newJournals) {
            ...
            try {
                await databases.createDocument(
                    "67e208370009f4c926ed",     // معرف قاعدة البيانات
                    "67e2085c001f7c56c20e",   // معرف المجموعة (Collection)
                    docId,
                    journal
                );
